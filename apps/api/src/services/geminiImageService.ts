...
STRICT RULES:
- The attached photo(s) are the identity reference(s) of the real person/people (in order: reference image 1, then 2). Reproduce each person's face with MAXIMUM likeness — the output face must be unmistakably the SAME person as their reference, faithful enough that a face-recognition system, or the person's own mother, would confirm the match. Copy the underlying facial features and geometry exactly: overall face shape, jawline, chin, cheekbones, forehead height and hairline; eye shape, size, spacing and iris color; eyebrow shape, thickness and arch; nose bridge width, length and tip; lip shape, volume and proportions; skin tone and undertone; and every distinguishing mark (moles, freckles, beauty marks, dimples) in the same places. Age, gender and ethnicity must read identical to the reference. Do NOT beautify, slim, smooth, or "idealize" the face — a prettified, younger, or averaged version that merely resembles them is a FAILURE.
- Copy the FEATURES only — NOT the reference photo's expression, gaze direction, head angle, lighting, or crop. The person's expression, pose, and framing come from the description above; render this exact person naturally taking on that expression and pose, and the expression must never warp, stretch, or genericize their facial features.
